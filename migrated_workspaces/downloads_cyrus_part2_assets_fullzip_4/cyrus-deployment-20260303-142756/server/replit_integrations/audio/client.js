 ...
 */
export async function voiceChat(audioBuffer, voice = "nova", // "nova" = sweet natural female voice for CYRUS
inputFormat = "wav", outputFormat = "mp3") {
 ...
 */
export async function voiceChatStream(audioBuffer, voice = "nova", // "nova" = sweet natural female voice for CYRUS
inputFormat = "wav") {
    const audioBase64 = audioBuffer.toString("base64");
 ...
 * Uses gpt-audio model via Replit AI Integrations.
 */
export async function textToSpeech(text, voice = "nova", // "nova" = sweet natural female voice for CYRUS
format = "wav") {
    const response = await openai.chat.completions.create({
        model: "gpt-audio",
        modalities: ["text", "audio"],
 ...
 * Note: Streaming only supports pcm16 output format.
 */
export async function textToSpeechStream(text, voice = "nova" // "nova" = sweet natural female voice for CYRUS
) {
    const stream = await openai.chat.completions.create({
        model: "gpt-audio",
        modalities: ["text", "audio"],
