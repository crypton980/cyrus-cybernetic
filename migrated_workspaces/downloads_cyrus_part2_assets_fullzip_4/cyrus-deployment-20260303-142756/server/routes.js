...
import { createAlpacaClient } from "./trading/alpaca-client";
...
const callOpenAIWithTimeout = async (fn, timeoutMs = 15000) => {
...
const normalizeActionType = (type) => {
    ...
    if (ALLOWED_ACTION_TYPES.includes(normalized)) {
        return normalized;
    }
    ...
    if (normalized.includes('double') && normalized.includes('click'))
        return 'double_click';
    if (normalized.includes('right') && normalized.includes('click'))
        return 'right_click';
    if (normalized.includes('key') || normalized === 'keypress')
        return 'press';
    if (normalized.includes('scroll'))
        return 'scroll';
    if (normalized.includes('type') || normalized.includes('text') || normalized.includes('input'))
        return 'type';
    ...
    if (normalized.includes('drag'))
        return 'drag';
    if (normalized.includes('navigate') || normalized.includes('url') || normalized.includes('goto'))
        return 'navigate';
    if (normalized.includes('search') || normalized.includes('find'))
        return 'search';
    if (normalized.includes('wait') || normalized.includes('pause') || normalized.includes('delay'))
        return 'wait';
...
export async function registerRoutes(httpServer, app) {
    ...
    app.post("/api/reminders", (req, res) => {
        ...
        if (!text || !time)
            return res.status(400).json({ error: "text and time required" });
    ...
    app.get("/api/news", async (req, res) => {
        const topics = req.query.topics || "general";
        ...
        try {
            ...
            if (!resp.ok)
                throw new Error(`news api ${resp.status}`);
    ...
    app.post("/api/scan/analyze", upload.single("file"), async (req, res) => {
        try {
            if (!req.file) {
                return res.status(400).json({ error: "No file uploaded" });
            }
    ...
    // QR-only decode
    app.post("/api/scan/qr", upload.single("file"), async (req, res) => {
        try {
            if (!req.file)
                return res.status(400).json({ error: "No file uploaded" });
            const buffer = req.file.buffer || (req.file.path ? await (await import("fs/promises")).readFile(req.file.path) : null);
            if (!buffer)
                return res.status(500).json({ error: "Unable to read uploaded file buffer" });
    ...
    app.post("/api/autonomy/execute", async (req, res) => {
        try {
            ...
            const report = await runAutonomy({ goal, context, modality }, {
    ...
    const executeAgentTask = async (command) => {
        ...
        try {
            ...
            const stepsReport = (task.steps || []).map((s, i) => `${i + 1}. ${s.feedback}`).join("\n");
            return {
...
If you detect a command that requires physical device interaction, inform the operator you're executing it.`;
            ...
            if (context) {
                if (context.detectedObjects && context.detectedObjects.length > 0) {
                    const objects = context.detectedObjects.map((o) => o.class).join(", ");
    ...
    const deviceState = {
        pointer: { x: 960, y: 540, isPressed: false, button: null },
        keyboard: { activeModifiers: [], lastKey: null, capsLock: false },
        clipboard: { currentContent: null, format: null },
        screen: { width: 1920, height: 1080, activeWindow: null }
    };
    ...
    const executeDeviceAction = (action) => {
        let executed = false;
        let result = "";
        let stateChange = {};
        ...
        switch (action.type) {
            ...
            case "hotkey":
                ...
                if (keys) {
                    ...
                    deviceState.keyboard.activeModifiers = keyList.filter((k) => ['ctrl', 'alt', 'shift', 'meta', 'cmd', 'command'].includes(k.toLowerCase()));
            ...
            case "release":
                ...
                if (releaseKey) {
                    deviceState.keyboard.activeModifiers = deviceState.keyboard.activeModifiers.filter(k => k.toLowerCase() !== releaseKey.toLowerCase());
    ...
    app.post("/api/cyrus/device/execute", async (req, res) => {
        try {
            ...
            let confidence = 0.95;
            try {
                const aiResponse = await callOpenAIWithTimeout((signal) => openai.chat.completions.create({
                    model: "gpt-4o",
                    messages: [
                        {
                            role: "system",
...
Return ONLY valid JSON.`
                ...
                }, { signal }), 10000 // 10 second timeout
                );
                let parsed = {};
                try {
                ...
                if (parsed.actions && Array.isArray(parsed.actions)) {
                    ...
                    parsedActions = parsed.actions.map((action, idx) => {
    ...
    const agentHistory = [];
    const agentFeedbackClients = [];
    // Helper function to send feedback to all connected clients
    const sendAgentFeedback = (feedback) => {
    ...
    const agentExecuteCore = async (command) => {
...
Return ONLY valid JSON.`
    ...
    app.get("/api/cyrus/agent/stream", async (req, res) => {
        ...
        req.on('close', () => {
            ...
            if (index > -1)
                agentFeedbackClients.splice(index, 1);
        });
    });
    ...
    app.get("/api/alpaca/quotes", async (req, res) => {
        ...
        try {
            const symbols = req.query.symbols?.split(',') || ['AAPL', 'GOOGL', 'MSFT', 'TSLA', 'AMZN'];
    ...
    app.get("/api/alpaca/crypto", async (req, res) => {
        ...
        try {
            const symbols = req.query.symbols?.split(',') || ['BTC/USD', 'ETH/USD', 'SOL/USD'];
