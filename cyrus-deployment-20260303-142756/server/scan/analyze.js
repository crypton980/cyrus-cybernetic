...
function classifyQrPayload(payload) {
    try {
        ...
        const suspicious = !url.protocol.startsWith("https") ||
...
export async function analyzeScan(buffer, declaredMime, opts = {}) {
    ...
    const confidence = !hasContent ? "Low" : languageConfidence > 0.7 ? "High" : languageConfidence > 0.4 ? "Medium" : "Low";
    const report = {
        success: hasContent,
        scanType,
