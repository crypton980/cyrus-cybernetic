function riskForAction(action) {
    if (action.includes("trade") || action.includes("device"))
        return "high";
    if (action.includes("analyze") || action.includes("summarize"))
        return "medium";
    return "low";
}
export function decomposeIntent(intent) {
    ...
    const execAction = intent.primaryDomain === "trading" ? "execute_trading" :
