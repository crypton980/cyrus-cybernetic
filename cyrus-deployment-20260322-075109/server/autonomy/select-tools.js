export function selectTools(steps) {
    return steps.map((step) => {
        let tool = "llm";
        if (step.action === "rag_retrieve")
            tool = "rag";
        if (step.action === "execute_trading")
            tool = "trading_service";
        ...
        if (step.action === "execute_device")
            tool = "device_controller";
        if (step.action === "execute_general")
            tool = "llm_orchestration";
