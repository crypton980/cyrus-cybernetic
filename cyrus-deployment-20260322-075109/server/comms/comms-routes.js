...
const router = Router();
router.get("/api/comms/users", isAuthenticated, async (req, res) => {
    try {
...
router.post("/api/comms/user/status", isAuthenticated, async (req, res) => {
    try {
        const userId = req.user?.claims?.sub;
...
router.get("/api/comms/messages/:recipientId", isAuthenticated, async (req, res) => {
    try {
        const userId = req.user?.claims?.sub;
        const { recipientId } = req.params;
        const messages = await db.select().from(directMessages)
            .where(or(and(eq(directMessages.senderId, userId), eq(directMessages.recipientId, recipientId)), and(eq(directMessages.senderId, recipientId), eq(directMessages.recipientId, userId))))
...
router.post("/api/comms/messages", isAuthenticated, async (req, res) => {
    try {
        const userId = req.user?.claims?.sub;
...
router.post("/api/comms/call/start", isAuthenticated, async (req, res) => {
    try {
        const userId = req.user?.claims?.sub;
...
router.post("/api/comms/call/:callId/answer", isAuthenticated, async (req, res) => {
    try {
...
router.post("/api/comms/call/:callId/end", isAuthenticated, async (req, res) => {
    try {
        const { callId } = req.params;
        await db.update(callHistory)
...
router.get("/api/comms/calls/history", isAuthenticated, async (req, res) => {
    try {
        const userId = req.user?.claims?.sub;
...
router.post("/api/comms/meeting/create", isAuthenticated, async (req, res) => {
    try {
        const userId = req.user?.claims?.sub;
        const { name } = req.body;
...
router.post("/api/comms/meeting/join", isAuthenticated, async (req, res) => {
    try {
        const userId = req.user?.claims?.sub;
...
router.get("/api/comms/meetings", isAuthenticated, async (req, res) => {
    try {
