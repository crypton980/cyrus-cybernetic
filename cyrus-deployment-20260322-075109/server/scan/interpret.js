...
const interpretClient = openaiApiKey && openaiBaseUrl
    ? new OpenAI({ apiKey: openaiApiKey, baseURL: openaiBaseUrl })
    : null;
export async function interpretText(text) {
    const warnings = [];
    if (!interpretClient) {
    ...
    try {
        const resp = await interpretClient.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "You are a multilingual intelligence analyst. Provide concise interpretation: key findings, risks/obligations, ambiguities. No fluff.",
                },
        ...
        for (const l of lines) {
            ...
            if (!interpretation && !lower.startsWith("-"))
                interpretation = l;
            else if (lower.includes("risk"))
                risks.push(l.replace(/^-+\s*/, ""));
            else if (lower.includes("ambigu"))
                ambiguities.push(l.replace(/^-+\s*/, ""));
            else
                keyFindings.push(l.replace(/^-+\s*/, ""));
        }
